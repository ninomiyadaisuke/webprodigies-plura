...
const PipelineTicket = ({ allTickets, index, setAllTickets, subaccountId, ticket }: Props) => {
  ...
  const handleClickEdit = () => {
    setOpen(
      <CustomModal subheading="" title="Update Ticket Details">
                ...
                <Card className="my-4 bg-white shadow-none transition-all dark:bg-slate-900">
                  <CardHeader className="p-[12px]">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {ticket.Tags.map((tag) => (
                        <TagComponent colorName={tag.color} key={tag.id} title={tag.name} />
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col justify-center">
                        ...
                        {ticket.assignedUserId && (
                          <span className="w-28 truncate  text-xs text-muted-foreground">{ticket.Assigned?.name}</span>
                        )}
                      </div>
                    </div>
